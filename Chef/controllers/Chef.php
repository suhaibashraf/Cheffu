...
function get_countries($order_by){
    ...
    $query = $this->mdl_chef->get_countries($order_by);
    return $query;
}
...
function insert_cities($cities){
    $this->load->model("mdl_chef");
    $this->mdl_chef->insert_cities($cities);
}

function get_cities($id){
    $this->load->model('mdl_chef');
    $query = $this->mdl_chef->get_cities($id);
    return $query;
}
...
function form_validate(){
        ...
        else
        {
            ...
            //$data_db_chef['img_url'] = "/uploads/dp/" . $file;
            $this->db->trans_start();
...
public function dashboard(){
    ...
    if(isset($_POST['chef_id'])){
        $data['chef_details'] = $this->get_where($_POST['chef_id']);
    }else{
        $data['chef_details'] = $this->get_where($_GET['attr']);
    }
    
    $this->load->module("User");
    if(isset($_POST['chef_id'])){
        $data['user_details'] = $this->user->get_where($_POST['chef_id']);
    }else{
        $data['user_details'] = $this->user->get_where($_GET['attr']);
    }
    $this->load->module("Notifications");
    if(isset($_POST['chef_id'])){
        $data['notifications'] = $this->notifications->get_where_unread($_POST['chef_id']);
    }else{
